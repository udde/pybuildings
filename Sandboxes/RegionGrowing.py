...
from Gui.Clustering import ClusteringBase




class RegionGrowing(ClusteringBase):
    ...
    def setupGui(self, layout):
        ...
        self.neigbour_slider.setMinimum(3)
...
def main ():
    ...
    #bfps, points, solids, property_area, mbb, top_dogs = data_handler.get_slice_from_property(1593) #Djupet 1593 #Diket 1592 1375 1343 1588 tät data: 1015 843 tre nivåer: 1594
    ...
    #printRegions(points[0], [region for region in regions if len(region) > 0])

    ##regions to file
    #f = open('regions2.csv', 'w')
    #for region in regions[0]:
    #    for idx in region:
    #        if idx == region[-1]:
    #            f.write(str(idx)+"\n")
